...
function calcIdentical(diceCount,N,s = 6){
  for(let i=d2i(s);i>=d2i(minDice);i--)
   if(diceCount[i]>=N) 
      return i2d(i)*N;
...
function calcTwoPairs(diceCount){
  let pair1 = calcIdentical(diceCount,2,6);
...
function printScoresHTML(play){
  ...
  for(let round=0;round<play.length;round++) {
    ...
    if(!isSpecialRound(round)){
      ...
      for(let d=0;d<play[round].diceRoll.length;d++){
         res+=`<img src="resources/${play[round].diceRoll[d]}-dice.png" width="20" height="20" alt="Picture of die with num ${play[round].diceRoll[d]}">`;
      }
      res+="</span>";
    }
    res+="</td>"
  ...
  res+='<tp class="hans">Heres a link to the <a href="HELP.html"> rules</a></tp>'
...
function createSelect(){
  let string2 = `
<form action="scores.html" method="get">
<label for="games">Choose a game:</label>
  <select name="games" id="game">
  <option value="yatzy">Yatzy</option>
  <option value="yatzy2">yatzy2</option>
  <input type="submit" value="Start spil">
</select>
</form>`;
let string =printHTMLHdr("index");
string += printHTMLBody(string2); 
fs.writeFileSync("index.html", string);
}
