...
function yatzyHighScorePage(formsData){
let i;
let new1 ='';
new1 +=`
<table id="scoretable">` 
if(yatzyController.games.length === 0){
  new1 += "<tr><td> no history </td></tr>"}
for(i = 0; i < yatzyController.games.length; i++){
  let currentgame = yatzyController.games[i];
  new1 +='<tr>'
  new1 +=`<th> Game ${i+1} </th>`;
  new1 +=`<td> ${currentgame.name} </td>`;
  new1 +=`<td> ${currentgame.scoreTable.score} </td>`
  new1 +="<br></br>"
  new1 +="</tr>"}; +
'</table>' 
;
let page = `${printHTMLHdr("Highscores",["css/style.css"])}
<h1>Highscores</h1>
`;
page += printHTMLBody(new1);
return page; //Add me
};
...
function YatzyGame(gameID,name,diceCount){
  ...
  this.printGameFormHTML= function (){
    ...
    <fieldset>
      <legend>Configure Game:</legend>
       ...
       <tp class="hans">Heres a link to the <a href="html/HELP1.html"> rules</a></tp>
  ...
  let highscore = yatzyHighScorePage();
  let newRoundFormHTML= `<h1> ${this.gameTitle}</h1>
  <form id="gameControl_id"  action="/nextround" method="post">
  <fieldset>
  <tp class="hans">Heres a link to the <a href="html/HELP1.html"> rules</a></tp>
  <tp class="hans2">, if you want to start a new game click <a href="http://127.0.0.1:3000/" "${(this.gameOver())}"> here</a></tp>
  ...
  <form id="gameControl_id" action="/highscores" method="get">
  <input type="submit" value="Highscores" ${(this.gameOver()) ? "" : "disabled"}>
  </form>`;
  let page=printHTMLHdr("IWP Yatzy Game",["css/style.css"]);
